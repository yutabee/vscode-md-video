 ...
 * with the muted <video> in the Markdown preview.
 *
 * WHY this exists: the M0 spike hard-set `audio.currentTime` to the video clock
 * on every `timeupdate` whenever drift passed a 0.25s threshold. Assigning
...
/**
 * Whether continuous drift correction should run on this tick. Skip while
 * either element is paused (nothing to keep in step — resume re-aligns) or
 * mid-seek (a seek is already converging; nudging/re-seeking would fight it and
 * re-flush the decode buffer, the very feedback loop driftAction exists to kill).
 * The caller applies this before reading the clocks in its `timeupdate` handler.
 */
export function shouldCorrectDrift(
  ...
  return !audioPaused && !videoPaused && !audioSeeking && !videoSeeking;
}
